...
def command(cmd : str) -> int:
    ...
    if str(lang) in dict_langs:
        ...
        match cmd:
            case '/c':
                print("Displayed current configuration")
            ...
            case '/m':
                ...
                if mode == ModeState.MULTIPLE_MODE:
                    print("Set current mode to Multiple .mp3 files successfully!!!")
                else:
                    print("Set current mode to One .mp3 files successfully!!!")
            case '/s':
...
def start() -> None:
    ...
    if mode == ModeState.DEFAULT_MODE:
        ...
        if os.path.exists(file_path):
            print(f"Input text file: {file_path}")
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                for index, line in enumerate(f):
                    ...
                    try:
                        ...
                        print(f"Generated: {name_file}_{index + 1}.mp3 | Text: {text}")
                        tts.save(f'{name_file}_{index + 1}.mp3')
                    except AssertionError as e:
                        print(f"Error generating {name_file}_{index + 1}.mp3: {e}")
            print("\033[92mGenerating successfully!!!\033[0m")
